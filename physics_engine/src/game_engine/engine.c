...
// Vector
float mod(vec2 *vec) { return sqrt((vec->x) * (vec->x) + (vec->y) * (vec->y)); }
...
void UpdatePostion(PhysicsObject *obj, float dt) {
  const vec2 velocity = {obj->current_pos.x - obj->prev_pos.x,
                         obj->current_pos.y - obj->prev_pos.y};
...
void ApplyConstraint(Solver *solver, PhysicsObject **objs, int size) {
  ...
  for (int i = 0; i < size; i++) {
    const vec2 to_obj = {objs[i]->current_pos.x - position.x,
                         objs[i]->current_pos.y - position.y};
...
void solveCollisions(Solver *solver, PhysicsObject **objs, int size) {
  for (int i = 0; i < size; i++) {
    ...
    for (int k = i + 1; k < size; k++) {
      ...
      const vec2 collision_axis = {obj_1->current_pos.x - obj_2->current_pos.x,
                                   obj_1->current_pos.y - obj_2->current_pos.y};
